            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-9 col-md-9 col-sm-8 col-xs-8 text" >
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:20px; margin-top: 10px">
                        <h3 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Rapport Journalier de vente Dépôt Rélais 2</h3>
                    </div>
            ...
            <div class="row spacer" >
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <table class="table table-bordered table-striped table-sm">
                        ...
                        <tbody>
                                <?php
                                $req = $bd->query("SELECT * FROM prodboiss AS A, catboiss AS B, depot AS C WHERE B.idCatBoiss=A.idCatBoiss AND A.idDepot = C.id AND A.idDepot = 2");
                                $num=1;
                                $totVente = 0;
                                $totProfit = 0;
                            ...
                            <tr>
                                    ...
                                    <?php 
                                        ...
                                        $req0 = $bd->query("SELECT * FROM `stock_init` AS A, `depot` AS B WHERE A.idBoiss ='$idB' AND A.dateStock='$dtJour' AND A.idDepot = B.id AND A.idDepot = 2");
                                        $don0=$req0->fetch();
                                        $stockInit = (($don0['qnteSt'] != NULL)?$don0['qnteSt']:$don['qnteBoiss']);

                                        //Calcul pour trouver Approv Buvette
                                        $req1 = $bd->query("SELECT SUM(qnteApprov) as qnteApprov FROM approvboiss AS A,depot AS B WHERE A.idBoiss = '$idB' AND A.idDepot = B.id AND A.dateApprov LIKE '%$dtJour%' AND A.idDepot = 2  ");
                                        $don1=$req1->fetch();

                                        //Calcul pour trouver Entree dans le depot
                                        $req2 = $bd->query("SELECT SUM(qnteSort) as qnteSort FROM sortieboiss AS A, depot AS B WHERE A.idDepot = B.id AND A.idBoiss = '$idB' AND A.dateSort ='$dtJour' AND A.idDepot = 2");
                                        $don2=$req2->fetch();
                                        ...
                                        $req3 = $bd->query("SELECT SUM(qteVendu) as qteVendu FROM ventedepot AS A, depot AS B WHERE A.idDepot = B.id AND A.idBoissVend = '$idB' AND dateVente ='$dtJour' AND A.idDepot = 2 ");
                                        $don3=$req3->fetch();
                ...
                <?php
                    ...
                    $req2 = $bd->query("SELECT * FROM users,depot WHERE idDepot = id AND user_function = 'Chef_Depot_Relais' AND idDepot = 2");
                    $don2=$req2->fetch(); 
