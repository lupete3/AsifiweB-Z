<?php
    ...
    $dateJ=date('Y-m-d');
    $req = $bd->prepare("SELECT date_format(A.dateApprovDiv,'%d-%m-%Y') as dateApprovDiv,A.qnteApprovDiv,A.puApprovDiv,B.designation FROM approvdiv as A,stockdivers as B WHERE A.idDiv = B.idDiv AND A.dateApprovDiv LIKE '%$dateJ%'");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(qnteApprovDiv*puApprovDiv) AS total FROM approvdiv as A,stockdivers as B WHERE A.idDiv = B.idDiv AND dateApprovDiv LIKE '%$dateJ%'");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT date_format(A.dateApprovDiv,'%d-%m-%Y') as dateApprovDiv,A.qnteApprovDiv,A.puApprovDiv,B.designation FROM approvdiv as A,stockdivers as B WHERE A.idDiv = B.idDiv AND A.dateApprovDiv BETWEEN ? AND ?");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(qnteApprovDiv*puApprovDiv) AS total FROM approvdiv as A,stockdivers as B WHERE A.idDiv = B.idDiv AND dateApprovDiv BETWEEN ? AND ?");
                        ...
                        <tbody>
                                ...
                                <tr>
                                       ...
                                       <td><?php echo $don['designation'] ?></td>
                                        <td><?php echo $don['puApprovDiv'].' Fc' ?></td>
                                       <td class="text-right"><?php echo $don['qnteApprovDiv'] ?></td>
                                       <td class="text-right"><?php echo $don['dateApprovDiv'] ?></td>

                                       
                                       <td class="text-right"><?php echo ($don['qnteApprovDiv']*$don['puApprovDiv'].' Fc') ?></td>
                                        <td></td>
