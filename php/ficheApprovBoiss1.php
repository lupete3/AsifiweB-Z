<?php
    ...
    $req = $bd->prepare("SELECT B.nomFour,date_format(A.dateApprov,'%d-%m-%Y') as dateApprov,A.qnteApprov,A.puApprov,C.designBoiss FROM approvboiss as A,four as B,prodboiss as C, depot AS D WHERE C.idBoiss=A.idBoiss AND B.idFour=A.idFour AND A.idDepot = D.id AND A.idDepot = 2 AND A.dateApprov LIKE '%$dateJ%'");
    $req->execute();

        $req1 = $bd->prepare("SELECT SUM(qnteApprov*puApprov) AS total FROM approvboiss AS A, depot AS B WHERE A.idDepot = B.id AND A.idDepot = 2 AND A.dateApprov LIKE '%$dateJ%'");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT B.nomFour,date_format(A.dateApprov,'%d-%m-%Y') as dateApprov,A.qnteApprov,A.puApprov,C.designBoiss FROM approvboiss as A,four as B,prodboiss as C, depot AS D WHERE C.idBoiss=A.idBoiss AND B.idFour=A.idFour AND A.idDepot = D.id AND A.idDepot = 2 AND A.dateApprov BETWEEN ? AND ?");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(qnteApprov*puApprov) AS total FROM approvboiss AS A, depot AS B WHERE A.idDepot = B.id AND A.idDepot = 2 AND dateApprov BETWEEN ? AND ? ");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h2 class="text-center text-uppercase" style="text-align:center; border:1px solid black">Fiche achat boisson dépôt rélais 2
