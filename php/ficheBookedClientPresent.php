            ...
            <div class="row spacer" >
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <table class="table table-bordered table-striped">
                        ...
                        $req = $bd->query("SELECT A.id_booking,A.modePaie, date_format(A.date_booking,'%d-%m-%Y %T') AS date_booking,date_format(A.date_in,'%d-%m-%Y ') AS date_in,date_format(A.date_out,'%d-%m-%Y ') AS date_out,B.id_client, C.room_code,C.room_prix,A.nombre_jour,A.reduction, A.total_apayer, A.accompte, A.reduction, A.reste_apayer, A.statut_booking, B.id_client, B.nom_client ,D.nomOrg FROM booking AS A, client AS B, rooms AS C , organisation AS D WHERE B.id_client = A.id_client AND C.id_room=A.num_chambre AND D.idOrg = B.idOrg AND A.statut_booking='CheckIn' ORDER BY A.id_booking DESC");

                        $req2 = $bd->query("SELECT sum(C.room_prix) AS totPrix,sum(A.reduction) AS totRed  FROM booking AS A, client AS B, rooms AS C , organisation AS D WHERE B.id_client = A.id_client AND C.id_room=A.num_chambre AND D.idOrg = B.idOrg AND A.statut_booking='CheckIn' ");
                            ...
                            <?php 
                                while ($don=$req->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo 1 ?></td>
                                        <td><?php echo $don['room_prix'] - $don['reduction'] ?></td>
                             ...
                             ?>
                             <tr>
                                <?php $don=$req2->fetch(PDO::FETCH_ASSOC); ?>
                                 <td colspan="4"><strong>Total Chambre</strong></td>
                                 <td ><strong><?php echo $don['totPrix'] - $don['totRed']; ?></strong></td>
                             </tr>
                        </tbody>
                    </table>
                </div>
