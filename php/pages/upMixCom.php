<?php
	...
	if($don=$req->fetch(PDO::FETCH_ASSOC)){
		...
		$req0=$bd->prepare('SELECT * FROM stock_init WHERE idBoiss=? AND dateStock = ?');
		$req0->execute(array($don['idBoiss'],date('Y-m-d')));
		...
		if(!$done){
			...
			$req1=$bd->prepare('INSERT INTO stock_init(dateStock,idBoiss,qnteSt) VALUES(?,?,?)');
