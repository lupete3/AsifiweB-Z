<?php 
	...
	$product='B';
	if ($c>$qteStock) {
		echo '<script>location.replace("venteRestaurant.php?msg=2")</script>';
		...
		$req=$bd->prepare("INSERT INTO panier(designation,qtePanier,idTable,puPanier,ptPanier,etatPanier,id_serveur,product,idPv,idBoiss)VALUES(?,?,?,?,?,?,?,?,?,?)");
		$req->execute(array($design,$c,$a,$pu,$pt,$etat,$o,$product,$point,$idBoiss));
		
		echo '<script>location.replace("venteRestaurant.php?msg=3")</script>';
	}
